...
exports.DeleteQuestion = exports.DeleteQuestions = exports.GetQuiz = exports.DeleteQuiz = exports.UpdateQuiz = exports.GetQuizzes = exports.CreateQuiz = void 0;
...
const CreateQuiz = async (req, res) => {
    try {
        ...
        if (await db_1.db.quiz.findUnique({ where: { title } })) {
            return res
                .status(400)
        ...
        const quiz = await db_1.db.quiz.create({
            data: {
                title,
                topicId,
            },
        ...
        const createdQuiz = await db_1.db.quiz.findUnique({
            where: {
                id: quizId,
            },
            include: {
...
const GetQuizzes = async (req, res) => {
    try {
        const getquizzes = await db_1.db.quiz.findMany({
            include: {
                questions: true,
                attempts: true,
            },
...
const UpdateQuiz = async (req, res) => {
    try {
        ...
        await db_1.db.$transaction(async (prisma) => {
                ...
                for (const question of questions) {
                    if (!question.question ||
                        ...
                        return res.status(400).json({
                            message: 'Each question must have a question text, at least 4 options, and an answer',
                        });
                    }
                    if (!question.options.includes(question.answer)) {
                        return res
            ...
            if (Object.keys(others).length > 0) {
                ...
                if (title &&
                    title !== quiz.title &&
                    !(await db_1.db.quiz.findUnique({ where: { title } }))) {
        ...
        const updatedQuiz = await db_1.db.quiz.findUnique({
            where: { id },
...
const DeleteQuestion = async (req, res) => {
    try {
        ...
        const question = await db_1.db.question.findUnique({
            where: { id: questionId },
        });
        if (!question) {
